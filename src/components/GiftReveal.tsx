...
export function GiftReveal({ partnerName = "Wanda", photos, onOpenGift }: GiftRevealProps) {
  ...
  return (
    <div className="fixed inset-0 z-[60] flex items-center justify-center bg-gradient-to-b from-[#5B0F14] via-[#2C070A] to-[#0D0204] px-5 py-6 font-serif text-white select-none overflow-y-auto">
      ...
      {stage === "gift" || stage === "opening" ? (
        ...
        <div key={index} className="animate-fade-in flex w-full max-w-md flex-col items-center">
          ...
          <button
            type="button"
            onClick={avancar}
            className="mt-6 rounded-full bg-[#C1121F] px-8 py-3.5 text-sm font-semibold uppercase tracking-widest text-white shadow-xl transition-all hover:scale-105 active:scale-95 border border-gold/30 cursor-pointer"
          >
